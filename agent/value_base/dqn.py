...
class DiscreteActionDQN(Agent):
    """
    Deep Q-Network (DQN) for discrete action (DA) space
    ...
    def __init__(self,
                 ...
                 params_out_path: str,
                 video_out_path: str):
        super(DiscreteActionDQN, self).__init__(name=name,
    ...
    def optimize_model(self, experiences: SARSTuple):
        ...
        max_a_q_sp = self.target_model(next_states).detach().max(dim=1)[0].unsqueeze(dim=1)
